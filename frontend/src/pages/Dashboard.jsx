...
/* Donut palette — teal, amber and ink accents for DealFlow AI CRM. */
const SOURCE_COLORS = ["#167f72", "#f59e0b", "#3fbfaa", "#475569", "#78d9c8", "#fbbf24"];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <h1 className="font-display text-3xl font-extrabold tracking-tight text-ink sm:text-[2.5rem]">
          Welcome back, <span className="text-brand-700">{user?.name?.split(" ")[0]}</span>
        </h1>
...
function EngagementChart({ trend }) {
  ...
  const renderPeak = (props) => {
    ...
    return (
      <g>
        <circle cx={cx} cy={y} r={5} fill="#167f72" stroke="#fff" strokeWidth={2} />
        <rect x={cx - 26} y={y - 34} width={52} height={22} rx={11} fill="#167f72" />
    ...
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={trend} barCategoryGap="28%" margin={{ top: 30 }}>
        ...
        <Bar dataKey="leads" radius={[14, 14, 14, 14]} maxBarSize={42} label={renderPeak}>
          {trend.map((t, i) => (
            <Cell key={i} fill={i === maxIndex ? "#167f72" : "#aeeade"} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
      ...
      <AreaChart data={trend} margin={{ top: 14, right: 0, left: 0, bottom: 0 }}>
        <defs>
          <linearGradient id="balance" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#249f8d" stopOpacity={0.3} />
            <stop offset="100%" stopColor="#249f8d" stopOpacity={0} />
          </linearGradient>
        </defs>
        <Tooltip content={<ChartTooltip prefix="$" />} />
        <Area type="monotone" dataKey="won" stroke="#167f72" strokeWidth={2.5} fill="url(#balance)" />
      </AreaChart>
    </ResponsiveContainer>
  );
}
